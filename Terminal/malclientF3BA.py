...
class MalClientF3BA(MalClient):
    def __init__(self, cfg, ID, train_dataset, test_dataset, identity="None"):
        ...
        # self.pattern = tmp_pattern.to(cfg.device)
        ...
        # data_idxs = range(len(self.train_dataset))
        nt = int(len(self.train_dataset) * 0.5)
        # train_ids, test_ids = data_idxs[:nt], data_idxs[nt:]
        # self.train_dataset = Subset(self.train_dataset, train_ids)
        handcraft_dataset = []
        ...
        # for i, batch in enumerate(self.train_dataset):
        #     if i < nt:
        #         train_ids.append([batch[0], batch[1]])
        #     else:
        #         test_ids.append([batch[0], batch[1]])
        # self.handcraft_dataset = test_ids
    def malicious_train(self, cfg):
        previous_global_model = copy.deepcopy(self.local_model)
        # print(f"|---  \n {(self.mask * self.pattern)[0, 3:7, 6: 9]}")

        self.handcraft(self.local_model, self.handcraft_loader, self.train_loader, previous_global_model)
    ...
    def optimize_backdoor_trigger(self, model: Model, candidate_weights, loader):
        ...
        for epoch in range(2):
            ...
            for i, data in enumerate(loader):
                ...
                # print(f"|---  \n {mal_imgs[0, 0, 3:7, 6: 9]}")
                # print(f"|---  \n {(mask * pattern)[ 0, 3:7, 6: 9]}")
                ...
                for h in range(n_channel):
                    pattern[h, x_top:x_bot, y_top:y_bot] = torch.clamp(pattern[h, x_top:x_bot, y_top:y_bot], 0,
                                                                       1, out=None)
    ...
    def conv_activation(self, model, layer_name, loader, attack):
        ...
        # print(len(loader),len(self.train_dataset))
        for i, data in enumerate(loader):
            imgs, labels = data
            imgs, labels = imgs.to(self.cfg.device), labels.to(self.cfg.device)
...
resnetactivations = ['layer1.0', 'layer1.1', 'layer1.2',
